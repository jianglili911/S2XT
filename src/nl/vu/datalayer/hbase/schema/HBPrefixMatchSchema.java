...
public class HBPrefixMatchSchema implements IHBaseSchema {
	...
	private static final String COPROCESSOR_CLASS_NAME =
			"nl.vu.datalayer.hbase.coprocessor.PrefixMatchGenerateSecondaryIndex";
	...
	public static long getCounterValue(long counterColumn, Configuration conf, String schemaSuffix) throws IOException{
		...
		Get g = new Get(COUNTER_TABLE.getBytes());
	...
	public static void updateCounter(int partitionId, long localRowCount, String schemaSuffix) throws IOException{
		...
		HTable h = new HTable(conf, COUNTER_TABLE+schemaSuffix);
		Put p = new Put(COUNTER_TABLE.getBytes());  // replace Put p = new Put(new byte[0]);
...
//			//
//			createSimpleTable(admin, ID2STRING+schemaSuffix, splits, COMPRESSED, RANDOM_ACCESS_PATTERN, ON_DISK/*IN_MEMORY*/);
//		}
//		else{
			createSimpleTable(admin, ID2STRING+schemaSuffix, splits, NO_COMPRESSION, RANDOM_ACCESS_PATTERN, ON_DISK/*IN_MEMORY*/);
//		}
	...
	final private static HTableDescriptor createTableDescriptor(String tableName, 
		...
		famDesc.setBloomFilterType(BloomType.ROW);
		//famDesc.setMaxVersions(1);
		if (compressionStatus == COMPRESSED){//by default it is disabled
			famDesc.setCompactionCompressionType(Algorithm.SNAPPY);
			famDesc.setCompressionType(Algorithm.SNAPPY);
		}
