...
public class BulkLoad extends AbstractPrefixMatchBulkLoad{
	...
	public BulkLoad(Path input, String outputPath, String schemaSuffix, boolean onlyTriples, int numberOfSlaveNodes) {
	...
	protected HBPrefixMatchSchema createPrefixMatchSchema() {
		return new HBPrefixMatchSchema(con, schemaSuffix, rdfUnitType==
				RDFUnit.TRIPLE, 2*numberOfSlaveNodes);
	}
	...
	protected void runResourceToTripleJob(Path resourceIds, Path convertedTripletsPath)
		...
		if (!fs.exists(convertedTripletsPath)) {
			...
			System.out.println("[Time] Second pass finished in: "+
	...
	public static void main(String[] args) {
		...
		try {
			if (args.length != 2){
				System.out.println("Usage: bulkLoad <inputPath>  <outputPath>");
				return;
			}		
			...
			boolean onlyTriples = Boolean.parseBoolean(prop.getProperty
					(HBPrefixMatchSchema.ONLY_TRIPLES_PROPERTY, ""));
			int slaveNodes = Integer.parseInt(prop.getProperty(
					HBPrefixMatchSchema.NUMBER_OF_SLAVE_NODES_PROPERTY, ""));
