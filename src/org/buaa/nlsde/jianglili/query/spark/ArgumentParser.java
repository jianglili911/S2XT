...
import org.buaa.nlsde.jianglili.utils.sparkactor.test;
...
public class ArgumentParser {
    ...
    public static void parseInput(String[] args) {
        ...
        Option schema = OptionBuilder.withArgName("file").hasArg()
                .withDescription("HDFS-Path to store RDF-Graph Scehma")
                .withLongOpt("schema").isRequired(false).create("s");
        ...
        try {
            ...
            formatter.printHelp("S2XSPARQL", options, true);
            Logger.getLogger(ArgumentParser.class).fatal(exp.getMessage(), exp);
            System.exit(-1);
        }
        ...
        if(cmd.hasOption("schema")){
            Const.schema_$eq(cmd.getOptionValue("schema"));
        }
