...
public class Generator {
  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      System.err.println("Usage: Generator\n" +
                         ...
                         "\t-path <output the generator file into path>") ;
      System.exit(0);
    }

    new Generator().start(univNum, startIndex, seed, daml, ontology,generatorPath);
  }
  ...
  public void start(int univNum, int startIndex, int seed, boolean daml,
                    String ontology, String generatorPath) {
  ...
  private void _generate(String generatorPath) {
    System.out.println("Started...");
    try {
//      log_ = new PrintStream(new FileOutputStream(System.getProperty("user.dir") +
//                                                 "\\" + LOG_FILE));
      log_ = new PrintStream(new FileOutputStream(generatorPath +
              "\\" + LOG_FILE));
      writer_.start();
      for (int i = 0; i < instances_[CS_C_UNIV].num; i++) {
        _generateUniv(i + startIndex_,generatorPath);
      }
      writer_.end();
      log_.close();
    }
   ...
   */
  private void _generateUniv(int index, String generatorPath) {
    ...
    for (int i = 0; i < instances_[CS_C_DEPT].num; i++) {
      _generateDept(index, i,generatorPath);
    }
  }
   ...
   */
  private void _generateDept(int univIndex, int index,String generatorPath ) {
//    String fileName = System.getProperty("user.dir") + "\\" +
//        _getName(CS_C_UNIV, univIndex) + INDEX_DELIMITER + index + _getFileSuffix();
    String fileName =generatorPath + "\\" +
            _getName(CS_C_UNIV, univIndex) + INDEX_DELIMITER + index + _getFileSuffix();
