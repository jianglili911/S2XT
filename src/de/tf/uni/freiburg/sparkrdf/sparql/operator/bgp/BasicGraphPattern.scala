...
import de.tf.uni.freiburg.sparkrdf.model.graph.node.{VertexInterface, _}
...
import de.tf.uni.freiburg.sparkrdf.sparql.operator.result.MyInt
...
trait BasicGraphPattern {
  ...
  protected def matchBGP(triples: Broadcast[java.util.List[TriplePattern]],
    ...
    val log: Logger = Logger.getLogger(classOf[QueryExecutor])
    ...
    val tripleAccumulator = context.accumulator(0)(MyInt)
    var messages = TriplePatternProcessor.matchBGP(tripleAccumulator,triples, g).cache()

    val messageCount=messages.count()   //
    log.log(Level.INFO,"Triple length: "+triples.value.length
      +" message count: "+messageCount+" tripleAccumulator:"+tripleAccumulator)

    if(tripleAccumulator.value!=Math.pow(2,triples.value.length)-1) {
      return  context.parallelize(Array.empty[(VertexId, VertexInterface)],1)
    }
    ...
    /*
       ...
       * after thar the one message will be joined back into the nodes of the graph. The nodes which get
       * messages from child or parent nodes are then used in the next step to start the first iteration.       *
       */
...
//    val vertexResults=vertexResultsP.coalesce(Const.dataNum,true)
//    log.log(Level.INFO,"vertexResults partition"+vertexResults.partitions.length)


 //   println("match partition:"+matching.partitions.length)
