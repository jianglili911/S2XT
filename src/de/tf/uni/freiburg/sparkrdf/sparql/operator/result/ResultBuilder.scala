...
trait ResultBuilder {
  ...
  protected def buildResult(triples: Broadcast[java.util.List[TriplePattern]], vertices: RDD[(VertexId, VertexInterface)]): RDD[SolutionMapping] = {
     ...
     */
    val vertexResults: RDD[SolutionMapping] = InitialSolutionMappingBuilder.findVertexResultNodes(toSearch, vertices)
     ...
     * I think the result will be only one partial Results
     */
