...
object SolutionMappingBuilder {
  ...
  def joinSolutionMappings(partialResults: java.util.List[PartialResult],
                           vertexResultsP: RDD[SolutionMapping]): scala.collection.mutable.Buffer[RDD[SolutionMapping]] = {


    val vertexResults=vertexResultsP.coalesce(Const.dataNum,false)
    ...
    partialResults.map(partialResult => {
      ...
      partialResult.getAllTriples().foreach(triplePattern => {
        ...
        if (firstRound) {
          ...
          solutionMappingForPartialResult = vertexResults.filter(result => result.getInitialTriplePattern()
...
//          log.log(Level.INFO,"SolutionMappingBuilder reduce begin"+solutionMappingForPartialResult.partitions.length)
//          val startLoading: Long = System.currentTimeMillis
          val isEmpty = solutionMappingForPartialResult.mapPartitions(iter => Iterator(!iter.hasNext), true).reduce(_ && _)
//          val endLoading: Long = System.currentTimeMillis - startLoading
//          log.log(Level.INFO,"SolutionMappingBuilder reduce end"+endLoading)
