...
import de.tf.uni.freiburg.sparkrdf.sparql.operator.join.Join
...
/**
  ...
  */
object SparkFacade extends BasicGraphPattern with ResultBuilder with Projection with LeftJoin with Join with Filter with Order {
  ...
  */
  ...
  //  conf.set("spark.driver.extraJavaOptions","-Xms0.5g -Xmx1g -XX:MaxPermSize=1024M");
...
//        "/" + Const.inputFile.replace("\\W", "_"))))
        if ((!fs.exists(new Path(Const.inputFile.replace("\\W", "_")+ "/vertices")))
          ||(!fs.exists(new Path(Const.inputFile.replace("\\W", "_")+ "/edges")))){
  ...
  /**
    * Save the result to the HDFS file
    */
  def saveResultToFile(rdd: RDD[SolutionMapping], filename:String) {
    if (rdd != null) {
      rdd.saveAsTextFile(Const.outputFilePath+filename);
    }
  }
  /**
    * Execute a left join
  ...
  /**
    * Join two RDDs
    */
  def join(joinVars: java.util.List[String],  left: RDD[SolutionMapping], right: RDD[SolutionMapping]): RDD[SolutionMapping] = {
    val broadcastVars: Broadcast[java.util.List[String]] = context.broadcast(joinVars)
    val result = join(broadcastVars, left, right)
    broadcastVars.unpersist(true)
    result
  ...
  def executeBasicGraphPattern(triples: java.util.List[TriplePattern]): RDD[(VertexId, VertexInterface)] = {
    // Broadcast all Sparql triples to all workers
    var broadcastedTriples: Broadcast[java.util.List[TriplePattern]] = context.broadcast(triples)
