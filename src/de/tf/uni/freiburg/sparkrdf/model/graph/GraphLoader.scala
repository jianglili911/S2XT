...
object GraphLoader extends Logging {
 ...
 // private var _originalGraphMap =new HashMap[String,Graph[VertexInterface, EdgeAttribute]]()
  ...
  def loadGraphCountBased(graphPath: String, context: SparkContext) {
    ...
    val nodes1: RDD[(VertexId, VertexInterface)] = zippedNodes.map(node => {
      val id = node._2
      val attribute = node._1
      (id, new VertexAttribute(attribute))
    })
    val nodes: RDD[(VertexId, VertexInterface)]=nodes1.coalesce(Const.dataNum/3,false)

    zippedNodes.unpersist(true)

    edges= edges.coalesce(Const.dataNum,true)

    _originalGraph = Graph(nodes, edges).persist(Const.STORAGE_LEVEL).partitionBy(PartitionStrategy.EdgePartition1D,Const.dataNum)
       // Save the result as object file
    if (Const.saveLoadOutput) {
      _originalGraph.vertices.saveAsObjectFile(Const.inputFile.replace("\\W", "_") + "/vertices")
      _originalGraph.edges.saveAsObjectFile(Const.inputFile.replace("\\W", "_") + "/edges")
    }
  }
  ...
  def loadGraphHashBased(graphPath: String, context: SparkContext) {
    ...
    // Get all nodes that are unique
    val distinctNodes: RDD[String] = context.union(subjects, objects).distinct.coalesce(Const.dataNum/3,false)
    ...
    edges= edges.coalesce(Const.dataNum,true)
    splitted.unpersist(true)
    _originalGraph = Graph(hashedNodes, edges).persist(Const.STORAGE_LEVEL).partitionBy(PartitionStrategy.EdgePartition1D,Const.dataNum)
  ...
  def loadPreloadedGraph(graphPath: String, context: SparkContext) {
    ...
    val nodes: RDD[(VertexId, VertexInterface)] = context.objectFile(Const.inputFile.replace("\\W", "_") + "/vertices")
    val edges: RDD[Edge[EdgeAttribute]] = context.objectFile(Const.inputFile.replace("\\W", "_") + "/edges")
    _originalGraph = Graph(nodes, edges).persist(Const.STORAGE_LEVEL)
    val attrs: RDD[(String,Long)]= edges.map(epre=>(epre.attr.getEdgeAttribute,1L)).reduceByKey(_+_)
    _originalGraph
  }
